...
class Reward:
    ...
    def get_rf(self):
        ...
        if self.total_angle <= 0:
            ...
            # 右下
            if self.total_angle < angle:
                rb = [loc_user_e[0] + xie * sin((angle - total_angle) * (pi / 180)),
                      loc_user_e[1] - xie * cos((angle - total_angle) * (pi / 180))]
            else:
                rb = [loc_user_e[0] - xie * sin((total_angle - angle) * (pi / 180)),
                      loc_user_e[1] - xie * cos((total_angle - angle) * (pi / 180))]

            # 左下
            if self.total_angle + angle < 90:
                lb = [loc_user_e[0] - xie * sin((angle + total_angle) * (pi / 180)),
                      loc_user_e[1] - xie * cos((angle + total_angle) * (pi / 180))]
            else:
                lb = [loc_user_e[0] - xie * cos((total_angle - 50) * (pi / 180)),
                      loc_user_e[1] + xie * sin((total_angle - 50) * (pi / 180))]
        # 左转
        else:
            # print('左转')
            total_angle = self.total_angle                      
          # 右上
            ...
            if self.total_angle + angle < 90:
                ...
                rb = [loc_user_e[0] + xie * cos((total_angle - 50) * (pi / 180)),
                      loc_user_e[1] + xie * sin((total_angle - 50) * (pi / 180))]
            ...
            if self.total_angle < angle:
                ...
                lb = [loc_user_e[0] + xie * sin((total_angle - angle) * (pi / 180)),
                      loc_user_e[1] - xie * cos((total_angle - angle) * (pi / 180))]
        ...
        k2 = 0.5
        if b_l < lu[0] < ru[0] < 0:
            self.rb = k2 * (1 - (abs(loc_user_e[0]) / 0.3))
        elif 0 < lu[0] < ru[0] < b_r:
            self.rb = k2 * (1 - (abs(loc_user_e[0]) / 0.3))
        else:
            self.rb = 0

        return self.rf, self.rc, self.rb
